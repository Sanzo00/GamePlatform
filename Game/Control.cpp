...
void Control::updateGrade()
{
    qDebug() << user_name << endl;
    dbHelper *db = new dbHelper;
    db->dbConnect();
    QSqlQuery query;
    QString sql = QString("select ugrade from %1.grades where gname = '飞机大战' and uname = '%2'").arg(db->dbName).arg(user_name);
    query.prepare(sql);
    query.exec();
    qDebug() << sql << endl;
    if (query.first() == false) { // 用户第一次玩, 需要新填数据
        sql = QString("insert into %1.grades(gname, uname, ugrade) values('飞机大战', '%2', %3)").arg(db->dbName).arg(user_name).arg(score);
        query.prepare(sql);
        query.exec();
        qDebug() << sql << endl;
        ...
        if (grade < score) {
            sql = QString("update %1.grades set ugrade = %2 where gname = '飞机大战' and uname = '%3'").arg(db->dbName).arg(score).arg(user_name);
            query.prepare(sql);
            query.exec();
        }
    }
